...
def con_home():
    ...
    ration_date=account[20]
    ration_time=account[21]

    return render_template('con_home.html',msg=msg,name=name,card=card,account=account)

@app.route('/con_purchase', methods=['GET', 'POST'])
def con_purchase():
    msg=""
    uname=""
    if 'username' in session:
    ...
    mycursor.execute('SELECT * FROM rq_time_queue WHERE card = %s', (uname, ))
    data = mycursor.fetchall()

    

    return render_template('con_purchase.html',msg=msg,name=name,card=card,data=data)
...
def admin():
    ...
    if act=="del":
        ...
        cursor.execute("delete from rq_ration where id=%s",(did,))
        mydb.commit()
        return redirect(url_for('admin'))

    if act=="del2":
        did=request.args.get("did")
        cursor.execute("delete from rq_employee where id=%s",(did,))
        mydb.commit()
        return redirect(url_for('admin'))
...
def view_card():
    ...
    if act=="del":
        did=request.args.get("did")
        cursor.execute("delete from rq_consumer where id=%s",(did,))
        mydb.commit()
        return redirect(url_for('view_card'))

    if act=="delall":
        
        cursor.execute("delete from rq_consumer where rid=%s",(rid,))
        mydb.commit()
        return redirect(url_for('view_card'))
...
def add_cat():
    ...
    if act=="del2":
        did=request.args.get("did")
        mycursor.execute("delete from rq_product where id=%s",(did,))
        mydb.commit()
        return redirect(url_for('add_cat'))   
    
    return render_template('add_cat.html',data=data,act=act)
